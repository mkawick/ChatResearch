...
static const char *Agricola_GameName = "agricola";
...
static void Agricola_OnReportGameResult(unsigned int gameID, int playerCount, unsigned int *pResults, unsigned int *pFactions)
{
   /*
...
#endif
*/
   const char *gameName = Agricola_GameName;
...
bool OnRequestUserProfileStats_Agricola(UserStatsMainThread *pUserStats, U32 connectionId, unsigned int profileUserId )
{
   ...
   response->gameType = GameProductId_AGRICOLA;
   ...
   if (res)
   {
      ...
      if (row)
      {
         ...
         if( row[13] != NULL )
         {
            response->userProfileStats.insert( "forfeits", row[13] );
         }
...
void Agricola_StatsPluginInit( StatsPlugin *pluginData, MYSQL *mysqlStats )
{
   ...
   pluginData->pGameName = Agricola_GameName;
   ...
   pluginData->onRequestUserProfileStats = OnRequestUserProfileStats_Agricola;
