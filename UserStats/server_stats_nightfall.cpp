...
static const char *Nightfall_GameName = "nightfall";
...
static void Nightfall_OnReportGameResult(unsigned int gameID, int playerCount, unsigned int *pResults, unsigned int *pFactions)
{
   /*
...
#endif
*/
   const char *gameName = Nightfall_GameName;
...
bool OnRequestUserProfileStats_Nightfall(UserStatsMainThread *pUserStats, U32 connectionId, U32 gatewayId, unsigned int profileUserId )
{
   ...
   response->gameType = GameProductId_NIGHTFALL;
   ...
   if (res)
   {
      ...
      if (row)
      {
         ...
         if( row[8] != NULL )
         {
            response->userProfileStats.insert( "wins_5p", row[8] );
         }
         if( row[9] != NULL )
         {
            response->userProfileStats.insert( "losses_5p", row[9] );
         }
         if( row[10] != NULL )
         {
            response->userProfileStats.insert( "forfeits", row[10] );
         }
...
void Nightfall_StatsPluginInit( StatsPlugin *pluginData, MYSQL *mysqlStats )
{
   ...
   pluginData->pGameName = Nightfall_GameName;
   ...
   pluginData->onRequestUserProfileStats = OnRequestUserProfileStats_Nightfall;
